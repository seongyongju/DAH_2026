...
```
  ① MUM-T 운용 환경 (표적)            유인 지휘부 ◀─ COP ─▶ UAV / UGV / SATCOM
        │                                    │
   ② 공격 킬체인 (§4)             ③ 방어: 탐지 → 차단 → 복구 (§5, 핵심)
   S1 링크침해 → S2 센서스푸핑            Zero-Trust COP + 팀 이중화(오버워치)
   → S3 노드장악 → S4 신뢰전파★                │  검증
   (+ S5 지속성/펌웨어)          ┌────────────┴────────────┐
        │                       ▼                          ▼
        └───────────▶  트랙 A: MUM-T 시뮬(RL)     트랙 B: 프로토콜 랩(Docker/Kali)
                              └──────────┬───────────────┘
                                         ▼  ④ 수행 주체
                          하이브리드 AI 에이전트
                          [ RL 코어 0.59µs ] + [ LLM 상위계층(교체형) ]
```

방어 에이전트 파이프라인: **DETECT → DECIDE(RL) → BLOCK → RECOVER** (자가대전 학습 되먹임).
상세 다이어그램은 `docs/DAH2026_예선보고서_본문_4-7.md`의 §0(Overview) · §6.3 참조.
